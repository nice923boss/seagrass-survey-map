...
| 專員編輯頁「儲存到網站」 | 頁面、拖曳、還原、匯出已在瀏覽器實測；儲存需 build 時注入 token，未經真實 PAT 實測 | 管理者建立 fine-grained PAT（僅此 repo、Contents: Read and write）並設 repo secret `EDITOR_GITHUB_TOKEN`，再跑一次 deploy（步驟見 README「專員點位編輯」）。未設定前按鈕停用，只能匯出檔案。 |
